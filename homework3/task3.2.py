# Дан список из 7 различных элементов.
# Используя функции (не использовать цикл), необходимо найти:
...
def get_list_info(lst):
    min_elem = int(min(lst))
    max_elem = int(max(lst))
    sum_list = int(sum(lst))
    average = float(round(mean(lst), 2))
